...
{
    LOG_DEBUG("OnImportFile: 开始导入文件");
    if (!m_playlistCtrl)
    {
        LOG_WARN("OnImportFile: m_playlistCtrl 为空");
        return;
    }
    ...
    if (added)
    {
        ...
        SavePlaylistConfig();
        LOG_INFO("OnImportFile: 已添加文件，播放列表大小: " << m_playlist_files.size());
    }
    LOG_DEBUG("OnImportFile: 完成");
}

void MainFrame::OnRemoveFile(wxCommandEvent &event)
{
    LOG_DEBUG("OnRemoveFile: 开始移除文件");
    if (!m_playlistCtrl)
    {
        LOG_WARN("OnRemoveFile: m_playlistCtrl 为空");
        return;
    }
    ...
    if (removedCurrent)
    {
        ...
        if (count > 0)
        {
            ...
            if (newViewIndex >= count)
                newViewIndex = count - 1;
    ...
    SavePlaylistConfig();
    LOG_INFO("OnRemoveFile: 完成，播放列表大小: " << m_playlist_files.size());
}

void MainFrame::OnClearList(wxCommandEvent &event)
{
    LOG_INFO("OnClearList: 清空播放列表");
    if (!m_playlistCtrl)
        return;
...
{
    LOG_DEBUG("OnPlaylistSelected: index=" << event.GetIndex());
    if (!m_playlistCtrl)
        return;
    if (m_is_programmatic_selection)
        return;

    long viewIndex = event.GetIndex();
    if (viewIndex < 0 || viewIndex >= m_playlistCtrl->GetItemCount())
        return;
    if (m_engine.is_playing())
    {
...
{
    LOG_DEBUG("OnPrev: 上一曲");
    if (m_playlistCtrl->GetItemCount() == 0)
        return;
...
void MainFrame::OnModeClick(wxCommandEvent &event)
{
    ...
    if (m_play_mode == wxString::FromUTF8("单曲播放"))
        m_play_mode = wxString::FromUTF8("单曲循环");
    else if (m_play_mode == wxString::FromUTF8("单曲循环"))
        m_play_mode = wxString::FromUTF8("列表播放");
    else if (m_play_mode == wxString::FromUTF8("列表播放"))
        m_play_mode = wxString::FromUTF8("列表循环");
    else if (m_play_mode == wxString::FromUTF8("列表循环"))
        m_play_mode = wxString::FromUTF8("随机播放");
    else
        m_play_mode = wxString::FromUTF8("单曲播放");
...
void MainFrame::OnPitchRangeChange(wxSpinEvent &event)
{
    ...
    if (minP > maxP)
    {
        if (event.GetId() == ID_MIN_PITCH_CTRL)
            maxP = minP;
        else
            minP = maxP;
...
void MainFrame::OnSaveKeymap(wxCommandEvent &event)
{
    ...
    if (ok)
        ...
        UpdateStatusText(wxString::FromUTF8("键位保存失败"));
    if (ok)
        SaveKeymapConfig();
}
...
{
    LOG_INFO("OnResetKeymap: 重置键位为默认值");
...
void MainFrame::UpdateTrackList()
{
    ...
    for (size_t i = 0; i < m_current_midi->tracks.size(); ++i)
    {
        ...
        if (track.note_count > 0)
        {
            ...
            if (name.IsEmpty())
                name = wxString::Format("Track %d", (int)i);
...
void MainFrame::SaveFileConfig()
{
    ...
    for (const auto &c : m_channelConfigs)
    {
        ...
        if (currentWindow.IsEmpty())
            currentWindow = wxString::FromUTF8("未选择");
        ...
        if (currentTrack.IsEmpty())
            currentTrack = wxString::FromUTF8("全部音轨");
...
{
    LOG_DEBUG("LoadGlobalConfig: 加载全局配置");
...
void MainFrame::LoadPlaylistConfig()
{
    ...
    wxCommandEvent dummy;
    OnSearch(dummy);
    LOG_INFO("LoadPlaylistConfig: 已加载 " << m_playlist_files.size() << " 个文件");
}
...
void MainFrame::SavePlaylistConfig()
{
    ...
    m_config->SetPath("/");
    m_config->Flush();
    LOG_DEBUG("SavePlaylistConfig: 完成");
}
...
void MainFrame::LoadKeymapConfig()
{
    ...
    if (!map.empty())
    {
        ...
        LOG_DEBUG("LoadKeymapConfig: 已加载 " << map.size() << " 个键位映射");
    }
}
...
void MainFrame::LoadFileConfig(const wxString &filename)
{
    ...
    for (auto &c : m_channelConfigs)
    {
        ...
        if (selWin != wxNOT_FOUND && selWin != 0)
        { // 0 is "None"
