...
namespace Core {
    ...
    static UINT GetCachedScanCode(int vk) {
        ...
        if (vk < 0 || vk > 255) return MapVirtualKey(vk, MAPVK_VK_TO_VSC);
    ...
    void KeyboardSimulator::send_input(int vk_code, int modifier, bool key_up) {
        ...
        if (!key_up) {
            ...
            // 1. Press Modifiers
            if (modifier == 1) add_input(VK_SHIFT, false);
            if (modifier == 2) add_input(VK_CONTROL, false);

            // 2. Press Main Key
            add_input(vk_code, false);

            // 3. Release Modifiers (Transient Strategy)
            if (modifier == 1) add_input(VK_SHIFT, true);
            if (modifier == 2) add_input(VK_CONTROL, true);
        } else {
            // Key Up Sequence
    ...
    void KeyboardSimulator::send_key_down(int vk_code, int modifier, void* hwnd) {
        ...
        if (hwnd) {
            ...
            auto send_msg = [&](int vk, bool up) {
                ...
                if (extended) lParam |= ((LPARAM)1 << 24);
            ...
            // 1. Press Modifiers
            if (modifier == 1) send_msg(VK_SHIFT, false);
            if (modifier == 2) send_msg(VK_CONTROL, false);

            // 2. Press Main Key
            send_msg(vk_code, false);

            // 3. Release Modifiers (Transient)
            if (modifier == 1) send_msg(VK_SHIFT, true);
            if (modifier == 2) send_msg(VK_CONTROL, true);
