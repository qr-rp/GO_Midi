...
namespace Core {
    ...
    void PlaybackEngine::load_midi(const Midi::MidiFile& midi_file) {
        ...
        // LOG("Loaded MIDI file. Total notes: " + std::to_string(m_all_notes.Size()) + 
        //    ", Duration: " + std::to_string(m_total_duration) + "s");
    ...
    void PlaybackEngine::seek(double time_s) {
        ...
        if (m_current_time < 0) m_current_time = 0;
        if (m_current_time > m_total_duration) m_current_time = m_total_duration;
    ...
    static int clamp_pitch(int pitch, int min_p, int max_p, bool smart) {
        int current = pitch;
        if (smart) {
            while (current < min_p) current += 12;
            while (current > max_p) current -= 12;
            if (current < min_p) current = min_p;
            if (current > max_p) current = max_p;
        } else {
    ...
    void PlaybackEngine::rebuild_events() {
        ...
        for (const auto& ch : m_channels) {
            if (ch->enabled) active_configs.push_back(ch.get());
        }
        ...
        for (const auto& raw : m_all_notes.GetVector()) {
            for (const auto& vc : m_valid_configs) {
                ...
                if (vc.is_specific_track) {
                    if (raw.track_index != vc.target_track) continue;
                } else {
                    // Global config: Skip percussion (Channel 10)
                    if (raw.channel == 10) continue;
                }
        ...
        m_events.GetVector().reserve(notes.size() * 2);
        {
            ...
            auto resolve = [&](TempNote* prev, TempNote* curr) {
                if (prev->pitch != curr->pitch) return; // Only resolve same pitch
        ...
        for (auto& note : notes) {
            if (note.end <= note.start) continue; // Skip invalid notes
        ...
        if (dropped_mapping_late > 0) {
            LOG("Late Mapping Stats: Dropped Mapping=" + std::to_string(dropped_mapping_late));
        }
        ...
        LOG("Rebuilt events. Total notes: " + std::to_string(m_all_notes.Size()) + 
            ", Filtered notes: " + std::to_string(notes.size()) + 
    ...
    void PlaybackEngine::playback_thread() {
        ...
        if (numProcessors > 0) {
            ...
            if (cpuIdx >= maxBits) cpuIdx = maxBits - 1;
            
            DWORD_PTR mask = (DWORD_PTR)1 << cpuIdx;
            DWORD_PTR result = SetThreadAffinityMask(GetCurrentThread(), mask);
            if (result == 0) {
                     // LOG("Failed to set thread affinity. Error: " + std::to_string(GetLastError()));
                } else {
                     // LOG("Set playback thread affinity to logical processor " + std::to_string(cpuIdx));
                }
            }
#endif
        ...
        while (m_running) {
            ...
            while (next_event_idx < m_events.Size()) {
                ...
                if (evt.time > m_current_time) break;
            ...
            if (sleep_ms >= 2.0) {
                 ...
                 while (true) {
                     ...
                     if (elapsed.count() >= sleep_ms) break;
