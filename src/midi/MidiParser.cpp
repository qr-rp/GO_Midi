...
namespace Midi {
    ...
    uint16_t MidiFile::readU16(size_t offset) {
        if (offset + 2 > m_data.size()) throw std::out_of_range("Offset out of range");
        return (static_cast<uint16_t>(m_data[offset]) << 8) | m_data[offset + 1];
    }

    uint32_t MidiFile::readU32(size_t offset) {
        if (offset + 4 > m_data.size()) throw std::out_of_range("Offset out of range");
    ...
    std::pair<uint32_t, size_t> MidiFile::readVarLen(size_t offset) {
        ...
        if (offset >= n) throw std::out_of_range("Unexpected EOF in varlen");
    ...
    std::string MidiFile::decodeText(size_t start, size_t len) {
        if (start + len > m_data.size()) return "";
        return std::string(reinterpret_cast<char*>(m_data.data() + start), len);
    }

    void MidiFile::parse() {
        if (m_data.size() < 14) throw std::runtime_error("Invalid MIDI file");
        if (memcmp(m_data.data(), "MThd", 4) != 0) throw std::runtime_error("Invalid MIDI header");

        uint32_t header_len = readU32(4);
        if (header_len < 6) throw std::runtime_error("Invalid MIDI header length");
        ...
        for (int i = 0; i < track_count; ++i) {
            if (pos + 8 > m_data.size()) break;
            if (memcmp(m_data.data() + pos, "MTrk", 4) != 0) throw std::runtime_error("Invalid track chunk header");

            uint32_t chunk_len = readU32(pos + 4);
            size_t chunk_start = pos + 8;
            size_t chunk_end = chunk_start + chunk_len;
            if (chunk_end > m_data.size()) throw std::runtime_error("Invalid track chunk length");
        ...
        for (size_t i = 0; i < parsed_notes_by_track.size(); ++i) {
            ...
            for (const auto& note : notes) {
                ...
                if (dur < 0.0) dur = 0.0;
                if (end_s > max_end) max_end = end_s;
    ...
    MidiFile::TrackParseResult MidiFile::parse_track(size_t start, size_t len, int track_index) {
        ...
        while (pos < end_pos) {
            ...
            if (status == 0xFF) { // Meta event
                ...
                if (meta_type == 0x58 && length >= 4) { // Time Signature
                    int nn = m_data[pos];
            ...
            if (event_type == 0x90) { // Note On
                ...
                if (vel == 0) { // Note Off via Note On with vel=0
        ...
        for (int i = 0; i < 2048; ++i) {
            if (note_start_depth[i] <= 0) continue;
    ...
    void MidiFile::init_tempo_map(const std::vector<std::pair<int, int>>& tempo_events) {
        ...
        for (const auto& ev : events) {
            if (ev.first == last_tick) {
                if (!deduped.empty()) deduped.back() = ev;
            } else {
    ...
    double MidiFile::tick_to_seconds(int tick) {
        ...
        if (m_tempo_ticks.empty()) return 0.0;
